...
This exemplary pipeline uses hit rate in a visual near-threshold task fluctuating across the respiratory cycle as an example for a behavioral outcome variable of interest. 
**Relevant Scripts**: ```Tutorial_DataPrep``` (MATLAB) / ```Tutorial_DataPÜrepMultiprocessing``` (Python) and ```Tutorial_DataClust``` (both)
...
This exemplary pipeline uses global field power in neural resting state data fluctuating across the respiratory cycle as an example for a neural outcome variable of interest, computing phase-amplitude coupling. Note that preprocessing/cleaning of continuous neural data is not the focus of this tutorial, thus we kindly refer the reader to the relevant *fieldtrip* and *MNE* tutorials for this step.
