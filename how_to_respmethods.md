...
**A note on convergence**: The IAAFT algorithm *iteratively* generates a new time series. Convergence is reached when the relative squared change between iterations drops below a criterion value. Theoretically, the function does not have a stop catch or warning for cases where this convergence criterion cannot be met - in practice, non-convergence or notably slow convergence are not a concern and only occur for very short time series (< 100 data points) or signals with an extremely narrow amplitude or frequency distribution (i.e. < 20 unique amplitude or frequency values). Note however that the phase congruence criterion can become a problem if the original time series is a very regular sinusoidal oscillation - the IAAFT may converge, but can fail to produce a sufficiently phase scrambled surrogate time series, trapping the surrogate generation code in an infinite loop. Based on our experience, we recommend manually stopping code execution after approx. 200 failed generation attempts in this case and increasing the value of the phase congruence criterion by 0.1 (Warning: we do not recommend a criterion > 0.3 to still satisfy chance-level surrogate phases). 
